...
static int iwl_fmac_get_temp_cmd(struct iwl_fmac *fmac, bool response,
				 s32 *temp)
{
	int ret;
	...
	if (response) {
		...
		/* fmac uses only extended format for older command versions */
		cmd.len[0] = sizeof(ext_cmd);
		cmd.data[0] = &ext_cmd;
	}
...
int iwl_fmac_get_temp(struct iwl_fmac *fmac, s32 *temp)
{
	...
	if (cmd_ver == 1)
		return iwl_fmac_get_temp_cmd(fmac, true, temp);
	...
	ret = iwl_fmac_get_temp_cmd(fmac, false, temp);
	if (ret) {
