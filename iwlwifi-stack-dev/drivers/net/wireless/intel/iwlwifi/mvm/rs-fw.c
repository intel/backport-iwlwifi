			      ...
			      u32 ampdu_size)
{
	...
	mvm = mvmsta->deflink.lq_sta.rs_fw.pers.drv;
	mvm_link_sta = rcu_dereference_protected(mvmsta->link[link_sta->link_id],
						 lockdep_is_held(&mvm->mutex));
	if (!mvm_link_sta) {
		IWL_DEBUG_RATE(mvm,
			       "Invalid mvmsta RCU pointer for link (%d) in set_ampdu_size\n",
			       link_sta->link_id);
		return -EINVAL;
	}
	lq_sta = &mvm_link_sta->lq_sta.rs_fw;

	ret = iwl_rs_send_dhc(mvm, lq_sta->pers.sta_id,
		     ...
		     enum nl80211_band band, bool update)
{
	...
	if (iwlwifi_mod_params.disable_11n & IWL_DISABLE_HT_TXAGG)
		iwl_rs_dhc_set_ampdu_size(sta, link_sta, 1);
#endif
	/*
